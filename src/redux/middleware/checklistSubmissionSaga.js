...
import { POST_SUBMISSION, FETCH_SUBMISSION, DOWNLOAD_SUBMISSION } from '../constants/actionTypes';
...
export function* watchDownloadSubmission() {
  console.log('working')
  yield takeLatest(DOWNLOAD_SUBMISSION, downloadSubmissionSaga);
}
    
export function* downloadSubmissionSaga(action) {
  try {
    const response = yield call(SubmissionAPI.downloadSubmission, `${action.secureUrl}`);
    yield put(fetchSubmissionSuccess(response.data));
  } catch (error) {
    const errorMessage = apiErrorHandler(error);
    yield put(fetchSubmissionFailure(errorMessage));
